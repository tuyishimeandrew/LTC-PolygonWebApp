...
if redo_file is None:
    ...
    # Sort by submission date & endtime, keep the last record for each Farmercode
    ...
    # Merge with the main dataframe to overwrite polygons if "redo_selectplot" matches
...
def parse_polygon_z(polygon_str):
    """
    Convert a text string of 'x y z' points separated by semicolons into a Shapely Polygon.
    Ignores Z (the third coordinate).
    """
...
def combine_polygons(row):
    """
    Combine all polygon columns (polygonplot, polygonplotnew_1, etc.)
    into a single geometry (union).
    """
    poly_list = []
...
def check_overlaps(gdf, target_code):
    ...
    The overall overlap is computed by unioning intersections with other polygons.
    """
    ...
    for _, row in gdf.iterrows():
        ...
        if other_poly and target_poly.intersects(other_poly):
            ...
            if overlap_area > 1e-6:  # Filter very tiny overlaps
                overlaps.append({
...
# Project to a planar CRS for area calculations
...
# ----------------------------
# HELPER: Function to plot geometry (handles Polygon, MultiPolygon, etc.)
# ----------------------------
...
# =========================================================
# Define the Risk Rating System
# =========================================================
def get_risk_rating(inconsistency_text):
    """
    Assign a risk rating based on the type of inconsistency.
    High risk => Overlap > 5%, Time < 15min but Registered == Yes
    Medium risk => phone mismatch, duplicates, or extremes in Productiveplants
    Low risk => everything else (including 'Plot lies within Uganda boundary')
    """
    text_lower = inconsistency_text.lower()

    # High-risk checks
    if "time < 15min" in text_lower:
        return "High"
    if "overlap > 5%" in text_lower:
        return "High"

    # Medium-risk checks
    if "phone mismatch" in text_lower:
        return "Medium"
    if "duplicate phone entry" in text_lower:
        return "Medium"
    if "duplicate farmer code" in text_lower:
        return "Medium"
    if "productiveplants exceed" in text_lower:
        return "Medium"
    if "productiveplants less" in text_lower:
        return "Medium"
...
def export_with_inconsistencies_merged(main_gdf, inconsistencies_df):
    """
    Converts main_gdf back to EPSG:4326, converts geometry to WKT,
    merges the inconsistency information (Risk Rating & Trust Responses) 
    with the main data, and exports a single sheet.
    """
    export_gdf = main_gdf.to_crs("EPSG:4326").copy()
    # Convert geometry to WKT for easy export
    export_gdf['geometry'] = export_gdf['geometry'].apply(lambda geom: geom.wkt)
    
    # Merge the risk columns based on Farmercode and username.
    # For rows without any inconsistency, default values are assigned.
    merged_df = export_gdf.merge(
        inconsistencies_df[['Farmercode', 'username', 'inconsistency', 'Risk Rating', 'Trust Responses']],
        on=['Farmercode', 'username'],
        how='left'
    )
    ...
    towrite.seek(0)
    st.download_button(
        label="Download Updated Inspection Form with Risk Columns",
        data=towrite,
        file_name="updated_inspection_form_merged.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

# Button to check Overlaps & Inconsistencies
if st.button("Check Overlaps & Inconsistencies"):

    # 1) Overlap results for the selected code (visual + stats)
    ...
    # Overlap map for the selected code
    if overlaps:
    ...
    # 2) Inconsistencies for the SELECTED code (basic info)
    ...
    # Convert phone columns to numeric -> int -> string
    ...
    # Check if plot is within a Uganda boundary polygon
    uganda_coords = [
        # Full set of Uganda boundary coordinates:
    ...
    if not gdf_code.empty:
        ...
        with st.expander("Uganda Polygon Inconsistency"):
            if not uganda_inconsistency.empty:
                ...
                st.write(uganda_inconsistency[['Farmercode', 'username', 'geometry']])
            else:
    ...
    if all(col in df.columns for col in required_inconsistency_cols):
        ...
        for _, row in df_phone_mismatch.iterrows():
            inconsistencies_list.append({
                'Farmercode': row['Farmercode'],
                'username': row['username'],
                'inconsistency': "Phone mismatch (Phone != Phone_hidden)"
            })

        df_dup_phones = df[df.duplicated(subset=['Phone'], keep=False)]
        for _, row in df_dup_phones.iterrows():
            inconsistencies_list.append({
                'Farmercode': row['Farmercode'],
                'username': row['username'],
                'inconsistency': "Duplicate phone entry"
            })

        df_dup_codes = df[df.duplicated(subset=['Farmercode'], keep=False)]
        for _, row in df_dup_codes.iterrows():
            inconsistencies_list.append({
                'Farmercode': row['Farmercode'],
                'username': row['username'],
    ...
    # 3c) Productiveplants check for entire dataset
    if 'Productiveplants' in df.columns:
        gdf_plants = gdf.copy()
        ...
        df_plants_incons = gdf_plants[gdf_plants['productiveplants'] > gdf_plants['expected_plants']]
        for _, row in df_plants_incons.iterrows():
            inconsistencies_list.append({
                'Farmercode': row['Farmercode'],
    ...
    # 3d) Uganda boundary check (flagging those within Uganda)
    df_uganda_incons = gdf[gdf['geometry'].within(uganda_poly)]
    for _, row in df_uganda_incons.iterrows():
        inconsistencies_list.append({
            'Farmercode': row['Farmercode'],
            'username': row.get('username', ''),
            'inconsistency': "Plot lies within Uganda boundary"
        })

    # 3e) Overlap check for ALL codes to see if union > 5%
    for code in farmer_codes:
        overlaps_all, overall_percentage_all = check_overlaps(gdf, code)
        if overall_percentage_all > 5:
            inconsistencies_list.append({
                'Farmercode': code,
                'username': '',
                'inconsistency': "Overlap > 5%"
            })

    # Build final DataFrame of inconsistencies with Risk & Trust columns
    if inconsistencies_list:
        df_incons_all = pd.DataFrame(inconsistencies_list)
        df_incons_all['Risk Rating'] = df_incons_all['inconsistency'].apply(get_risk_rating)
        df_incons_all['Trust Responses'] = df_incons_all['Risk Rating'].apply(lambda x: "No" if x == "High" else "Yes")
        st.dataframe(df_incons_all)
    else:
        st.write("No inconsistencies found across the entire dataset.")

    # --- EXPORT BUTTON ---
    if st.button("Export Updated Form to Excel (Merged with Risk Columns)"):
        if 'df_incons_all' in locals():
            export_with_inconsistencies_merged(gdf, df_incons_all)
        else:
            export_with_inconsistencies_merged(gdf, pd.DataFrame([]))
